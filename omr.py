...
with open('input/files.txt') as f:
    lines = f.readlines()
THRESHOLD = 0.9
def grade(inputfile):
    ...
    # inputfile = "bai3.jpg"
    # colorImage = Image.open(inputfile)
    ...
    def check_stone_coor(x,y):
        return (x <= 100 or x >= 800) and (y <=150 or y>= 1000)
    ...
    for contour in contours:
        ...
        if w >= 25 and w <= 45 and h >= 25 and h<= 45 and check_stone_coor(x,y):
        # if w>=800 and h>=1000:
...
#     stones3 = np.array(stones3)
    ...
    def plot_ans(ans, x1, y1, color = (255,0,0), size=5):
    ...
    made = ''.join(map(lambda x: str(x), cmd))
    ...
    plot_ans(answers, ax1, ay1, (0, 255, 0), 3)
    ...
    plt.imshow(g)
    plt.axis('off')
    plt.savefig(f'./output/{inputfile}_{grade}.jpg', bbox_inches='tight', dpi=400, transparent=True, pad_inches=0)
    # plt.show() 
    plt.close()
